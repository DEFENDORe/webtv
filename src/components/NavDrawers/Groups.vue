<template>
  <VResizeDrawer ref="resizeDrawer" v-model="drawers.groups" :location="'left'" width="300px" min-width="200px"
    max-width="50%" permanent :touchless="xs" :save-width="false" :style="resizeDrawerStyle">
    <template v-slot:handle>
      <v-icon icon="mdi-chevron-right" />
    </template>

    <template v-slot:prepend>
      <v-text-field prepend-inner-icon="mdi-magnify" density="compact" label="Search Groups" variant="solo" hide-details
        single-line clearable persistent-clear v-model="groupFilter"></v-text-field>
    </template>
    ...
    <v-virtual-scroll v-else :items="filteredGroups" :height="`calc(100dvh - 106px - ${drawers.video ? 300 : 0}px)`">
      <template v-slot:default="{ item }">
        <v-list-item @click="selectedGroup = item; drawers.groups = !xs"
          :title="item === '__NULL__' ? '(UNSORTED)' : item" :active="selectedGroup === item">
